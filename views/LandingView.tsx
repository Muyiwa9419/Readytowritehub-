    ...
    <div className="space-y-24 md:space-y-40 pb-20">
      ...
      <section className="min-h-[70vh] md:min-h-[85vh] flex flex-col items-center justify-center text-center space-y-8 md:space-y-10 animate-in fade-in zoom-in duration-1000 relative">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6 pt-4 w-full sm:w-auto px-6">
          <button 
            onClick={onEnter}
            className="w-full sm:w-auto px-8 md:px-12 py-4 md:py-5 bg-white text-slate-950 rounded-full font-bold text-base md:text-lg hover:bg-indigo-400 hover:text-white transition-all hover:scale-105 active:scale-95 shadow-2xl shadow-indigo-500/30 flex items-center justify-center gap-3"
          >
            Enter the Silence
        ...
        <div className="pt-8 md:pt-12 animate-in fade-in duration-1000 delay-500 px-6 w-full max-w-sm">
           <div className="glass-card px-6 md:px-8 py-4 rounded-3xl border border-white/5 relative group">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-indigo-500 text-[7px] md:text-[8px] font-bold uppercase tracking-[0.2em] rounded-full">
                Midnight Muse
              </div>
              <p className="text-indigo-300/80 text-[10px] md:text-xs italic serif leading-relaxed">
                {loadingMuse ? "Summoning inspiration..." : `"${musePrompt}"`}
              </p>
              <button 
                onClick={fetchPrompt}
                className="mt-2 text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-slate-600 hover:text-indigo-400 transition-colors"
              >
                Refresh Insight
              </button>
           </div>
      ...
      <section id="manifesto" className="max-w-6xl mx-auto px-6 space-y-12 md:space-y-20">
        <div className="text-center space-y-4">
          <h3 className="text-3xl md:text-5xl font-bold text-white">The Midnight Manifesto</h3>
          <p className="text-slate-500 serif italic text-sm md:text-base">Why we write when the world sleeps.</p>
        </div>
      ...
      <section className="bg-white/[0.02] py-20 md:py-32 border-y border-white/5">
        <div className="max-w-7xl mx-auto px-6 space-y-12 md:space-y-16">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-6 text-center md:text-left">
            <div className="space-y-2">
              <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-indigo-400">Preview</span>
              <h3 className="text-3xl md:text-4xl font-bold text-white leading-tight">Curated Whispers</h3>
              <p className="text-slate-500 text-sm md:text-base serif">A glimpse into our latest reflections.</p>
            </div>
            <button 
              onClick={onEnter}
              className="text-white font-bold text-xs md:text-sm border border-white/10 px-6 md:px-8 py-2 md:py-3 rounded-full hover:bg-white hover:text-slate-950 transition-all"
            >
              View Full Feed
            </button>
          </div>
      ...
      {/* About Section */}
      <section id="about" className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center px-6">
        <div className="relative group order-2 md:order-1">
          ...
          <div className="absolute -bottom-4 -right-4 md:-bottom-6 md:-right-6 glass-card p-4 md:p-6 rounded-2xl border border-white/10 animate-float">
            <p className="text-white font-bold text-xs md:text-sm">"Words are the advocate's truest evidence."</p>
            <p className="text-[9px] md:text-[10px] text-indigo-400 mt-1 uppercase tracking-widest">— Mosunmola, Esq</p>
          </div>
        </div>
        <div className="space-y-6 md:space-y-8 order-1 md:order-2 text-center md:text-left">
          <div className="space-y-3 md:space-y-4">
            <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.3em] text-indigo-400">The Brain Box</span>
            <h3 className="text-3xl md:text-5xl font-bold text-white leading-tight">Advocate by day, Whisperer by night.</h3>
          </div>
          ...
          <div className="flex justify-center md:justify-start gap-8 md:gap-10 pt-4 md:pt-6">
            <div>
              <p className="text-2xl md:text-3xl font-bold text-white">Esq</p>
              <p className="text-[9px] uppercase tracking-widest text-slate-500">Professional Identity</p>
            </div>
            <div>
              <p className="text-2xl md:text-3xl font-bold text-white">Writer</p>
              <p className="text-[9px] uppercase tracking-widest text-slate-500">Creative Soul</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6">
        <div className="glass-card p-8 md:p-20 rounded-[2.5rem] md:rounded-[3.5rem] border border-white/10 relative overflow-hidden text-center space-y-6 md:space-y-8">
          ...
          <div className="space-y-3 md:space-y-4">
            <h3 className="text-3xl md:text-4xl font-bold text-white">Join the Midnight Mailing</h3>
          ...
          <p className="text-[9px] text-slate-600 uppercase tracking-widest">No noise. No clutter. Just words.</p>
        </div>
      </section>
    </div>
  );
