...
const PostView: React.FC<PostViewProps> = ({ post, onBack, onUpdateStats, onNewComment }) => {
  ...
  const shareText = `Reading "${post.title}" on readytowritehub`;
  ...
  const handleDislike = () => {
    ...
    if (userLiked === 'dislike') {
      ...
      if (userLiked === 'like') newLikes = Math.max(0, newLikes - 1);
      newDislikes += 1;
      setUserLiked('dislike');
      localStorage.setItem(`rtwh_user_vote_${post.id}`, 'dislike');
    }
    onUpdateStats({ likes: newLikes, dislikes: newDislikes });
  };
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (onNewComment) onNewComment(`Whisper shared & dispatched.`);
  ...
  return (
    <article className="max-w-3xl mx-auto animate-in fade-in duration-700">
      ...
      <button onClick={() => { stopReading(); onBack(); }} className="mb-8 flex items-center gap-2 text-slate-400 hover:text-white transition-colors text-sm group">
        <span className="group-hover:-translate-x-1 transition-transform">←</span>
        Back to the silence
      </button>

      <div className="rounded-3xl overflow-hidden mb-12 shadow-2xl shadow-black/50 aspect-video relative group">
        ...
        <div className="absolute bottom-6 left-6 flex items-center gap-4">
          <button 
            ...
            className={`flex items-center gap-3 px-6 py-3 rounded-full font-bold text-xs uppercase tracking-widest transition-all ${isReading ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-white/10 backdrop-blur-md text-white border border-white/10 hover:bg-white/20'}`}
          >
            {audioLoading ? (
            ...
            {audioLoading ? 'Summoning Voice...' : isReading ? 'Silence the Reading' : 'Listen to Whisper'}
          </button>
          {isReading && (
            <div className="flex gap-1 animate-pulse">
              <div className="w-0.5 h-3 bg-indigo-400" />
              <div className="w-0.5 h-5 bg-indigo-400" />
      ...
      <header className="mb-12">
        <div className="flex items-center gap-4 mb-6">
          <span className="bg-indigo-500/10 text-indigo-400 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border border-indigo-500/20">{post.category}</span>
          <span className="text-slate-500 text-xs">•</span>
          <span className="text-slate-500 text-xs italic">{post.readingTime} read</span>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-indigo-300 font-bold text-lg">{post.author[0]}</div>
            <div>
          ...
          <div className="flex flex-wrap items-center gap-3">
             <button onClick={copyToClipboard} className={`flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 transition-all text-xs font-bold group ${copySuccess ? 'text-green-400 border-green-500/30' : 'text-slate-400 hover:text-white'}`}>
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>
              {copySuccess ? 'Copied!' : 'Share Whisper'}
            </button>
          </div>
        </div>
      </header>

      <div className="prose prose-invert prose-lg max-w-none text-slate-300 serif leading-relaxed space-y-6">
        {post.content.split('\n').map((para, i) => para ? <p key={i}>{para}</p> : <br key={i} />)}
      </div>

      <div className="mt-20 p-8 glass-card rounded-3xl border border-white/5 space-y-8 animate-in slide-in-from-bottom-4 duration-1000">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="space-y-4 text-center md:text-left">
            <h4 className="text-white font-bold text-lg">How did this feel?</h4>
          ...
          <div className="flex items-center gap-6">
            <button onClick={handleLike} className={`flex flex-col items-center gap-2 group transition-all ${userLiked === 'like' ? 'text-indigo-400' : 'text-slate-500'}`}>
              <div className={`p-4 rounded-full border border-white/5 transition-all group-hover:scale-110 ${userLiked === 'like' ? 'bg-indigo-500/10 border-indigo-500/40' : 'hover:bg-white/5'}`}>
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M2 20h2c.55 0 1-.45 1-1v-9c0-.55-.45-1-1-1H2v11zm19.83-7.12c.11-.25.17-.52.17-.8V11c0-1.1-.9-2-2-2h-5.5l.92-4.38c.02-.11.03-.23.03-.35c0-.42-.17-.8-.45-1.07L13.96 2L8.59 7.37C8.23 7.73 8 8.24 8 8.8V18c0 1.1.9 2 2 2h6.91c.73 0 1.38-.4 1.72-1.02l3.2-7.4c.1-.25.17-.52.17-.8l-.17.1z"/></svg>
              </div>
      ...
      <section className="mt-12 space-y-12">
        <div className="space-y-4">
          ...
          <form onSubmit={handleCommentSubmit} className="space-y-4 glass-card p-6 rounded-3xl border border-white/5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Your Name</label>
                <input type="text" value={newComment.author} onChange={e => setNewComment({ ...newComment, author: e.target.value })} placeholder="Anonymous Dreamer" className="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 text-white text-sm" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Your Thought</label>
